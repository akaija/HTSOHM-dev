...
def load_and_subs_template(template_name, values):
    htsohm_dir = os.path.dirname(htsohm.__file__)
    template_path = os.path.join(htsohm_dir, 'simulation', 'forcefield', template_name)
...
def write_cif_file(material, simulation_path):
    ...
    """
    values = {'CellLengthA' : material.structure.lattice_constant_a,
              'CellLengthB' : material.structure.lattice_constant_b,
              'CellLengthC' : material.structure.lattice_constant_c}
    cif_template = load_and_subs_template('template.cif', values)

    file_name = os.path.join(simulation_path, '{}.cif'.format(material.uuid))
