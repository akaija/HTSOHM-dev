...
def create_unit_cell(a, b, c, seed):
    ...
    #add_cube(location = (dim[0], dim[1], dim[2]))
...
def add_atom_site(x, y, z, radius,  material, seed, site_count):
    add_sphere(location = (x, y, z), size = radius * 0.5)
...
def render_from_seed(seed, color_by, tag, view):
    print("seed   :\t{}".format(seed))
    seed = int(seed)

    # query run_id
#    run_id = session.query(Material.run_id).filter(Material.seed==seed).one()[0]
    run_id = "2019-01-06T23:22:31.937227"
...
#    m, s = generate_material(run_id, seed, config)
    m, s = generate_material("test", seed, config)
    ...
    #eps = [at.epsilon for at in s.atom_types]
    #ep_lims = [min(eps), max(eps)]
    ep_lims = [0, 450]
...
#        print("{} of at least {}".format(site_count, len(s.atom_sites)))
    ...
    if view == "angle":
        bpy.ops.object.camera_add(location=(a + 10., b + 10., c + 10.), rotation=(0., 0., 0.))
    ...
    if view == "angle":
        ...
        bpy.context.object.constraints["Track To"].target = bpy.data.objects["UnitCell_{}".format(seed)]
...
#    bpy.ops.wm.save_mainfile(filepath = blender_file)
    ...
    png_file = os.path.join(pwd_dir, "man003/renders/{}_{}_{}_{}.png".format(tag, seed, color_by, view))
